module.exports = class mysqlhelper{
    ...
    */
    ...
    get_auth_request(fn){
        let query = "select auth_request.request_id, username, name, id_number, description from auth_request inner join submitar on auth_request.request_id = submitar.request_id inner join user on submitar.auth_user_id = user.user_id where state = 0";
        let params = [];
        this.connection.query(query, params, (err, res) => {
            if (err) {
                console.log('[error] - ', err.message);
