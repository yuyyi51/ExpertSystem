module.exports = class MongoDB{
    ...
    get_expert_id(username, fn){
        this.mongo_client.connect(this.db_url, (err, cli) => {
            ...
            cli.db().collection('author').findOne({user:username}, (err, res) => {
                if (err) {
                    console.log('[error] - ', err.message);
                    cli.close();
                    fn(null);
    ...
    change_info(username, info, fn){
        this.mongo_client.connect(this.db_url, (err, cli) => {
            ...
            cli.db().collection("author").updateOne({user:username},{$set:info},(err, res) => {
                if (err) {
                    console.log('[error] - ', err.message);
                    cli.close();
                    fn(null);
